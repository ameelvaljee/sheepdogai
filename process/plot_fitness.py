...
def graph(variant, runs=20, generations=150):
  ...
  for prefix in AGGREGATE_PREFIXES:
    ...
    if "mhom" in prefix:
      ...
      p = "SHOM"
    if variant in ["het", "hom", "ahet", "het-d", "hom-d", "ahet-d"]:
      if "-e" in prefix:
        color = "g"
      elif "-m" in prefix:
        color = "b"
    ...
    elif variant in [ "het-easymap", "hom-easymap", "ahet-e"]:
      if "-e-e" in prefix:
        color = "g"
      elif "-m-e" in prefix:
        color = "b"
    ...
    elif variant in [ "het-medmap", "hom-medmap", "ahet-m"]:
      if "-e-m" in prefix:
        color = "g"
      elif "-m-m" in prefix:
        color = "b"
    ...
    plt.figure(1)
    plt.plot(AGGREGATE_MEAN_ARRAY, label=p, c=color, ls=style)

    plt.figure(2)
    plt.plot(AGGREGATE_MAX_ARRAY, label=p, c=color, ls=style)
